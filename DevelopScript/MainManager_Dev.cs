...
/*==============================
    ステージの全体管理
===============================*/
public class MainManager_Dev : MonoBehaviour
{
    int nextStage;                      //ステージKey格納
    float total;                        //確率合計値格納
    Transform cameraPos;                //カメラの位置
    GameObject stage;                   //リソース抽出用
    GameObject instance;                //インスタンス生成用
    List<GameObject> list;              //プレイ中のステージ管理用
    Dictionary<int,string>stageInfo;    //ステージ辞書
    Dictionary<int,float>sponeInfo;     //確率辞書

    public List<GameObject> stageList;         //エディタ上のステージ管理用
    public List<float> probability;   //エディタ上のステージ管理用
    ...
    void Update(){
        if(camera_Con.start){
            ...
            StopCoroutine(stageManage());
        }
    }
    //プレイヤーより背後のステージ削除
    //動的ステージ生成
    IEnumerator stageManage(){
        if(list[0].transform.position.z + 3 <= cameraPos.position.z){
            Destroy(list[0]);
            nextStage = Respone();
    ...
    //各ステージ生成（重み付き確立）
    int Respone(){
        float random = Random.value * total; //(0.0~1.0) * 100
        //randomに該当するキーを返す
    ...
    void InitializeDicts(){
        stageInfo = new Dictionary<int,string>();   //ステージを格納
        sponeInfo = new Dictionary<int,float>();    //確率の重みを格納
        for(int i=0;i<stageList.Count;i++){         //各辞書に登録
            stageInfo.Add(i,stageList[i].name);
            sponeInfo.Add(i,probability[i]);
        }
        //全体の確率(100%計算)
        foreach(KeyValuePair<int,float> elem in sponeInfo){
            total += elem.Value;
    ...
    /*ステージ生成(旧式ver４.5)
        stageInfo = new Dictionary<int,string>();//ステージ格納
        ...
        stageInfo.Add(16,"blindStage");stageInfo.Add(17,"itemStage_Dev");

        sponeInfo = new Dictionary<int,float>();//確率重み格納
        for(int i=0;i<probability.Count;i++){
            sponeInfo.Add(i,probability[i]);
        }
        sponeInfo.Add(0,49.0f); sponeInfo.Add(1,3.0f);
        ...
        sponeInfo.Add(16,2.0f);sponeInfo.Add(17,4.0f);*/
